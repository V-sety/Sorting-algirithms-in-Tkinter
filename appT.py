...
import random
from Algorithms import bubbleSort, insertionSort, selectionSort, quickSort
...
#row0
Label(UI_frame, text ="Choose algorithm: ", bg ="pink").grid(row = 0, column = 0, padx = padx, pady = pady, sticky =W)
algMenu = ttk.Combobox(UI_frame, textvariable=selectedAlg, values = ["Bubble Sort", "Insertion Sort", "Selection Sort", "Quick Sort"])
algMenu.grid(row = 0, column = 1, padx = padx, pady = pady)
algMenu.current(0)

Button(UI_frame, text = "Generate array", command = generateArr, bg = "dark red").grid(row = 0, column = 2,padx = padx, pady = pady)
Button(UI_frame, text = "Start", command = startSort, bg = "dark red").grid(row = 0, column = 3,padx = padx, pady = pady)
#row1
Label(UI_frame, text ="Array size ", bg ="pink").grid(row = 1, column = 0, padx = padx, pady = pady, sticky =W)
sizeEntry = Entry(UI_frame)
sizeEntry.grid(row = 1, column = 1, padx = 5, pady = 5, sticky =W)

Label(UI_frame, text ="Minimum value ", bg ="pink").grid(row = 1, column = 2, padx = padx, pady = pady, sticky =W)
minEntry = Entry(UI_frame)
minEntry.grid(row = 1, column = 3, padx = 5, pady = 5, sticky =W)

Label(UI_frame, text ="Maximim value ", bg ="pink").grid(row = 1, column = 4, padx = padx, pady = pady, sticky =W)
